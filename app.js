...
(function() {
    ...
    function accurateInterval(fn, time) { let nextAt, timeout; const wrapper = () => { nextAt += time; timeout = setTimeout(wrapper, nextAt - Date.now()); fn(); }; const start = () => { nextAt = Date.now() + time; timeout = setTimeout(wrapper, time); }; const stop = () => clearTimeout(timeout); return { start, stop }; }
    ...
    function playMetronomeClick() { try { if (!metronomeAudioCtx) metronomeAudioCtx = new (window.AudioContext || window.webkitAudioContext)(); if (metronomeAudioCtx.state === 'suspended') metronomeAudioCtx.resume(); const oscillator = metronomeAudioCtx.createOscillator(), gainNode = metronomeAudioCtx.createGain(); oscillator.type = 'square'; oscillator.frequency.setValueAtTime(1000, metronomeAudioCtx.currentTime); gainNode.gain.setValueAtTime(1.0, metronomeAudioCtx.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.001, metronomeAudioCtx.currentTime + 0.05); oscillator.connect(gainNode); gainNode.connect(metronomeAudioCtx.destination); oscillator.start(); oscillator.stop(metronomeAudioCtx.currentTime + 0.05); } catch (e) { console.warn("Metronome click failed", e); } }
    function getEstimated1RM(exerciseName) { const filteredLogs = workoutLogs.filter(log => log.exercise.toLowerCase().trim() === exerciseName.toLowerCase().trim()); if (filteredLogs.length === 0) return null; const maxE1RM = Math.max(...filteredLogs.map(log => { const weight = parseFloat(log.weight); const reps = parseInt(log.reps); if (isNaN(weight) || isNaN(reps) || weight <= 0 || reps <= 0) return 0; return weight * (1 + (reps / 30)); })); return maxE1RM > 0 ? maxE1RM : null; }
    async function addLog(log) { workoutLogs.unshift(log); const tx = db.transaction(LOG_STORE_NAME, 'readwrite'); const store = tx.objectStore(LOG_STORE_NAME); await new Promise((resolve, reject) => { const request = store.add(log); request.onsuccess = resolve; request.onerror = reject; }); addLogToView(log); populateExerciseDropdown(); updateChart(); }
    async function deleteLog(id) { if (!confirm("Delete this logged set?")) return; const tx = db.transaction(LOG_STORE_NAME, 'readwrite'); const store = tx.objectStore(LOG_STORE_NAME); await new Promise((resolve, reject) => { const request = store.delete(id); request.onsuccess = resolve; request.onerror = reject; }); const logIndex = workoutLogs.findIndex(log => log.id === id); if (logIndex > -1) { workoutLogs.splice(logIndex, 1); } const elementToRemoveToday = DOMElements.logList.querySelector(`[data-log-id="${id}"]`); if (elementToRemoveToday) elementToRemoveToday.remove(); const elementToRemoveHistory = DOMElements.historyList.querySelector(`[data-log-id="${id}"]`); if (elementToRemoveHistory) { const parentGroup = elementToRemoveHistory.parentElement; elementToRemoveHistory.remove(); if (parentGroup && parentGroup.children.length === 0) { parentGroup.parentElement.remove(); } } if (DOMElements.logList.children.length === 0) renderTodaysLogs(); if (DOMElements.historyList.children.length === 0) renderHistory(); populateExerciseDropdown(); updateChart(); }
    function createLogItemDOM(log) { const item = document.createElement('div'); item.className = "flex justify-between items-center bg-zinc-800 p-3 rounded-lg border border-zinc-700/60"; item.setAttribute('data-log-id', log.id); const exerciseDiv = document.createElement('div'); exerciseDiv.className = "font-semibold text-white"; exerciseDiv.textContent = log.exercise; const controlsDiv = document.createElement('div'); controlsDiv.className = "flex items-center gap-4"; const statsSpan = document.createElement('span'); statsSpan.className = "text-yellow-500 font-bold"; statsSpan.textContent = `${log.weight} lbs × ${log.reps}`; const deleteButton = document.createElement('button'); deleteButton.className = "text-red-400 hover:text-red-500 font-bold px-2 py-1 text-sm transition"; deleteButton.title = "Delete set"; deleteButton.innerHTML = '✕'; deleteButton.onclick = () => deleteLog(log.id); controlsDiv.append(statsSpan, deleteButton); item.append(exerciseDiv, controlsDiv); return item; }
    function addLogToView(log) { const placeholder = DOMElements.logList.querySelector('p'); if (placeholder) placeholder.remove(); const logItem = createLogItemDOM(log); DOMElements.logList.prepend(logItem); }
    ...
    function populateExerciseDropdown() { const select = DOMElements.exerciseSelect; const currentSelection = select.value; const uniqueExercises = [...new Set(workoutLogs.map(log => log.exercise.toLowerCase().trim()))].sort(); select.innerHTML = '<option value="">Select Exercise...</option>'; uniqueExercises.forEach(ex => { const opt = document.createElement('option'); opt.value = ex; opt.textContent = ex.charAt(0).toUpperCase() + ex.slice(1); select.appendChild(opt); }); if (uniqueExercises.includes(currentSelection)) { select.value = currentSelection; } }
    function updateChart() { const selectedExercise = DOMElements.exerciseSelect.value; if (progressChart) progressChart.destroy(); if (!selectedExercise) return; const filteredLogs = workoutLogs.filter(log => log.exercise.toLowerCase().trim() === selectedExercise); const maxWeightPerDate = {}; const max1RmPerDate = {}; filteredLogs.forEach(log => { const weight = parseFloat(log.weight); const reps = parseInt(log.reps); if (isNaN(weight) || isNaN(reps) || weight <= 0 || reps <= 0) return; const estimated1RM = Math.round(weight * (1 + (reps / 30))); if (!maxWeightPerDate[log.date] || weight > maxWeightPerDate[log.date]) { maxWeightPerDate[log.date] = weight; } if (!max1RmPerDate[log.date] || estimated1RM > max1RmPerDate[log.date]) { max1RmPerDate[log.date] = estimated1RM; } }); const sortedDates = Object.keys(maxWeightPerDate).sort((a, b) => new Date(a) - new Date(b)); const weightData = sortedDates.map(date => maxWeightPerDate[date]); const oneRmData = sortedDates.map(date => max1RmPerDate[date]); Chart.defaults.color = '#9ca3af'; progressChart = new Chart(DOMElements.progressChartCtx, { type: 'line', data: { labels: sortedDates, datasets: [{ label: 'Max Weight (lbs)', data: weightData, borderColor: '#eab308', backgroundColor: 'rgba(234, 179, 8, 0.15)', borderWidth: 3, pointBackgroundColor: '#eab308', pointBorderColor: '#000', fill: true, tension: 0.3 }, { label: 'Estimated 1RM (lbs)', data: oneRmData, borderColor: '#e5e7eb', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], pointBackgroundColor: '#e5e7eb', pointBorderColor: '#000', fill: false, tension: 0.3 }] }, options: { responsive: true, scales: { y: { beginAtZero: false, grid: { color: 'rgba(63, 63, 70, 0.5)' } }, x: { grid: { display: false } } }, plugins: { legend: { display: true, labels: { color: '#d1d5db', usePointStyle: true } } } } }); }
    function completeStep(index, exerciseName) { const weightInput = document.getElementById(`wt-${index}`); const repsInput = document.getElementById(`rp-${index}`); const weight = weightInput.value.trim(); const reps = repsInput.value.trim(); if (!weight || !reps) return alert('Please input actual weight and reps.'); const log = { id: Date.now(), date: getISODate(), exercise: exerciseName, weight, reps }; addLog(log); if (DOMElements.autoTimerCheck.checked) { resetTimer(); startTimer(); if (metronomeInterval) toggleMetronome(); } const stepDiv = document.getElementById(`step-${index}`); const btn = document.getElementById(`btn-complete-${index}`); stepDiv.classList.add('opacity-30', 'pointer-events-none'); btn.classList.replace('text-transparent', 'text-black'); btn.classList.replace('border-zinc-600', 'border-yellow-500'); btn.classList.replace('bg-zinc-950', 'bg-yellow-500'); }
    ...
    function setupEventListeners() { DOMElements.loadButton.addEventListener('click', () => DOMElements.restoreInput.click()); DOMElements.restoreInput.addEventListener('change', restoreBackup); DOMElements.saveButton.addEventListener('click', backupData); DOMElements.navTabs.forEach(tab => tab.addEventListener('click', () => switchView(tab.dataset.view))); DOMElements.addTimeBtn.addEventListener('click', () => addTime(60)); DOMElements.resetTimerBtn.addEventListener('click', resetTimer); DOMElements.metronomeBtn.addEventListener('click', toggleMetronome); DOMElements.activeRoutineSelect.addEventListener('change', renderRoutineChecklist); DOMElements.exerciseSelect.addEventListener('change', updateChart); DOMElements.addExerciseRowBtn.addEventListener('click', addExerciseRow); DOMElements.saveRoutineBtn.addEventListener('click', saveRoutine); DOMElements.modalCloseBtn.addEventListener('click', closeExerciseModal); DOMElements.modalOverlay.addEventListener('click', closeExerciseModal); DOMElements.routineChecklist.addEventListener('click', (e) => { const button = e.target.closest('button'); if (!button) return; const { action, index, exerciseName } = button.dataset; if (action === 'show-info') openExerciseModal(exerciseName); if (action === 'complete') completeStep(index, exerciseName); if (action === 'skip') skipStep(index); }); DOMElements.exerciseRowsContainer.addEventListener('click', (e) => { const button = e.target.closest('.remove-row-btn'); if (button) button.parentElement.remove(); }); DOMElements.routineList.addEventListener('click', (e) => { const button = e.target.closest('button[data-action="delete-routine"]'); if (button) deleteRoutine(parseInt(button.dataset.id, 10)); }); DOMElements.historyList.addEventListener('click', e => { const button = e.target.closest('button[data-action="delete-log"]'); if (button) deleteLog(parseInt(button.dataset.id, 10)); }); }
