...
(function() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(LOG_STORE_NAME)) {
                    ...
                    // Create an index on the 'date' property for efficient querying
    ...
    // STATE MANAGEMENT
    let workoutLogs = []; // This will now act as a temporary in-memory cache
    let workoutRoutines = [];
    ...
    const DOMElements = {
        ...
        logCustomSetBtn: document.getElementById('logCustomSetBtn'),
    ...
    // ... (rest of the file remains largely the same, but with updated data functions) ...
    ...
    function saveRoutine() {
        ...
        saveRoutines(); // Now calls the specific routine saver
    ...
    async function init() {
        try {
            await openDB(); // Open the database first
            await loadData(); // Then load the data
    ...
    // --- All other functions (UI, timer, etc.) remain the same ---
    // (For brevity, only the changed/relevant functions are shown in full detail above)
    // The following are stubs for the rest of the functions that don't need to change.
    
    async function requestWakeLock() { try { if ('wakeLock' in navigator && (!wakeLock || wakeLock.released)) { wakeLock = await navigator.wakeLock.request('screen'); console.log('Screen Wake Lock acquired'); } } catch (err) { console.warn(`Wake Lock error: ${err.name}, ${err.message}`); } }
    function releaseWakeLock() { if (wakeLock !== null && !wakeLock.released) { wakeLock.release().then(() => { wakeLock = null; console.log('Screen Wake Lock released'); }); } }
    ...
    function accurateInterval(fn, time) { let nextAt; let timeout; const wrapper = () => { nextAt += time; timeout = setTimeout(wrapper, nextAt - Date.now()); fn(); }; const start = () => { nextAt = Date.now() + time; timeout = setTimeout(wrapper, time); }; const stop = () => clearTimeout(timeout); return { start, stop }; }
    ...
    function saveGenericSet() { const exercise = DOMElements.custom.exercise.value.trim(); const weight = DOMElements.custom.weight.value; const reps = DOMElements.custom.reps.value; if (!exercise || !weight || !reps) return alert('Please fill in all fields!'); const log = { id: Date.now(), date: getISODate(), exercise, weight, reps }; addLog(log); DOMElements.custom.weight.value = ''; DOMElements.custom.reps.value = ''; if (DOMElements.autoTimerCheck.checked) { resetTimer(); startTimer(); if(metronomeInterval) toggleMetronome(); } }
    ...
    function addExerciseRow() { /* ... unchanged ... */ }
